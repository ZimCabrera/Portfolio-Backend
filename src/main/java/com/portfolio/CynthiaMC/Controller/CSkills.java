...
@CrossOrigin(origins = "https://front-end-a7c57.web.app")
public class CSkills {
    ...
    public ResponseEntity<Skills> getById(@PathVariable("idSkill") int idSkill){
    ...
    public ResponseEntity<?> update(@PathVariable("idSkill") int idSkill, @RequestBody dtoSkills dtoskills){
        if(!sSkills.existsById(idSkill))
    ...
    public ResponseEntity<?> delete(@PathVariable("idSkill") int idSkill){
        if(!sSkills.existsById(idSkill))
            return new ResponseEntity(new Mensaje("El ID no existe"), HttpStatus.NOT_FOUND);
